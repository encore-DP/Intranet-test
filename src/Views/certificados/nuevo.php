...
<html lang="es">
    ...
    <?php include __DIR__ . '/../layouts/sidebar.php'; ?>            
                                    ...
                                    <div class="card-body">
                                            <div class="col-sm-12">
                                                <select id="alumno_id" name="alumno_id" class="form-select" required>
                                                  ...
                                                  <?php foreach ($alumnosN as $a): if(!$a['id']) continue; ?>
                                                    <option value="<?= htmlspecialchars($a['id']) ?>">
                                                      <?= htmlspecialchars($a['nombre']) ?> (<?= htmlspecialchars($a['dni']) ?>)
                                                    </option>
                                                  <?php endforeach; ?>
                                                </select>
                                            </div>
                              ...
                              <div class="card">
                                ...
                                <div class="card-body">
                                    <select id="curso_id" name="curso_id" class="form-select" required>
                                        <option value="">— Selecciona —</option>
                                        <?php foreach ($cursosN as $c): if(!$c['id']) continue; ?>
        ...
        <script>
        (() => {
          const alumnos = <?= json_encode($alumnosN ?? []) ?>;
          const cursos  = <?= json_encode($cursosN ?? []) ?>;
        
          const byId = (arr, id) => arr.find(x => String(x.id) === String(id)) || null;
        
          document.getElementById('alumno_id')?.addEventListener('change', e => {
            const a = byId(alumnos, e.target.value);
            document.getElementById('alumno_nombre').value  = a ? a.nombre  : '';
            document.getElementById('alumno_dni').value     = a ? a.dni     : '';
            document.getElementById('alumno_empresa').value = a ? a.empresa : '';
          });
      
          document.getElementById('curso_id')?.addEventListener('change', e => {
            const c = byId(cursos, e.target.value);
